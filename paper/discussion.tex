...
spatial form is consistent local theory (\S\ref{fargo_m1}). Then to a
first approximation, we can regard it   
as a mode within the framework of local theory, satisfying the
% temperature gradient ignored in local theory
dispersion relation Eq. \ref{dispersion}. To demonstrate possible
instability, we show that the torque density acting on such a mode due
to the background temperature gradient is negative, which enforces the
mode, because it has negative angular momentum.  
