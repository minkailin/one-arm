...
%torque effective for large scales or sharp gradients

%Then, since $dc_s^2/dR < 0$, $T_\mathrm{BG}<0$ so the background
...
The background torque density $T_{BG}$ does not depend on
self-gravity. 
%and by definition, disc structure plays no role in the local description of linear modes 
However, our numerical simulations show that the $m=1$ spiral is
...
structure. Thus, the role of disc structure and self-gravity is to
