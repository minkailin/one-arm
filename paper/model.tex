...
The 2D Poisson
equation is solved in integral form using Fast Fouier Transform. This
requires a softening length $\epsilon_g H$, where $\epsilon_g$ is an
input parameter. 
...
overall picture of disc evolution. In the next section, we explore the
effect of disc parameters using 2D simulations. 

The disc evolution is quantified using mode amplitudes and angular
momenta as follows. We list the 2D definitions with obvious 3D
generalisations. The hydrodynamic variables are written as 
\begin{align}
  f(R,\phi) &= \sum_{m=-\infty}^{\infty}f_m(R)\exp{\ii m \phi} \notag\\
  &= f_0 + 2 \real\left[\sum_{m=1}^\infty f_m \exp{(\ii
      m\phi)}\right], 
\end{align}
where the $f_m$ may be obtained from Fourier transform in $\phi$. The
mode amplitude is 
\begin{align}
  A_m = \frac{|C_m|}{M_{d0}}
\end{align}
...
\begin{align}
  ...
  \sum_{m=1}^\infty2\pi\int_{R_1}^{R_2} 2R\real\left[\Sigma_m v_{\phi
      m}^*\right] RdR, \notag\\
  &= \sum_{m=0}^\infty 2\pi \int_{R_1}^{R_2} j_m R dR = \sum_{m=0}^\infty J_m. 
\end{align}
where $^*$ denotes complex conjugation, and we idenfity $J_m$ as the angular momentum 
associated with the $m^\mathrm{th}$ mode, and $j_m$ its density. 

\subsection{WKB perturbations}
We will use results from linear density wave theory to set up
perturbations. 
Here, we follow conventions in \cite{shu91}. Recall the
dispersion relation for tightly-wound density waves of the form
$\exp{(\ii \omega t - m \phi + kR)}$ in a razor-thin
isothermal disc, 
\begin{align}\label{dispersion}
  (\omega - m\Omega)^2 = \kappa^2 + k^2c_s^2 - 2\pi G \Sigma |k|, 
\end{align}
where $\omega$ is the mode frequency, $\kappa^2 =
R^{-3}\p_R(R^4\Omega^2)$ is the square of the epicycle frequency, and
$k$ is a real wavenumber. 

If we specify the pattern speed $\Omega_p \equiv \omega/m$,
