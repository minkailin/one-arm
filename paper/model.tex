...
stability of disc equilibra described 
...
\cite{shu91}.   In particular, we highlight a key feature of
locally isothermal discs that enables angular momentum exchange
between linear perturbations and the background disc. 
...
be positive or negative), where $\bm{\xi}$ is the Lagrangian
...
momentum flux. % and the perturbations are regular. 
...
exerted by the background disc on the perturbation. We outline the
derivation of $T_\mathrm{BG}$ in Appendix \ref{tbg_deriv}. 

The important consequence of the background torque is the possibilty
of instability if $\tbg\jlin>0$. That is, if $\jlin$ is positive
(negative) and the background torque $\tbg$ is positive (negative),
then the local angular momentum density of the linear disturbance
further increases (decreases). This implies the amplitude of the disturbance
...
\section{Numerical methods}\label{methods}
