...
{\bf 
  We demonstrate instability for low-frequency
  ($\omega\ll m\Omega$) perturbations by explicitly calculating its 
  angular momentum density 
  $\jlin$ and the background torque $T_\mathrm{BG}$, and show that
  they are unstable when a negative radial 
...
momentum. 
%The local approximation becomes less good for global modes,
...
\ref{horizontal_displacements}.} 
 % {\bf
% This holds for local modes of all frequencies and can be shown
% explicitly by evaluating Eq. \ref{ang_mom_def} with 
% \begin{align}
%   \xi_\phi = -\frac{2\ii\Omega}{\sbar}\xi_R, 
% \end{align}
% consistent with the local approximation as shown in Appendix
% \ref{horizontal_displacements}. 
% }
Then 
...
{\bf It is important to note that, although our description of the local
...
% {\bf The bump function mimics mass built-up in $R\in[R_1,R_2]$.  
