...
% In this paper we will mainly use non-linear hydrodynamic simulations to investigate the
% stability of disc equilibria described 
% above. However, it is useful to  interpret simulation results
% using linear density theory. We list here the main definitions and
% results from 2D linear theory, taken from \cite{lin11b} and
% \cite{shu91}.  
{\bf We highlight a key feature of locally isothermal discs that
  ...
  For simplicity, we consider 2D discs, although generalisation to 3D is straight
  forward.  
}

In a linear analysis, one assumes a steady axisymmetric background state, % such as
% that defined in \S\ref{model},
...
$\gamma$ the growth rate, and $m$ is an integer. Here, we take $m>0$ without
loss of generality.  
%Since the background is
...
{\bf numerical} exploration of the linear problem to a future study. Here, we {\bf
  discuss} some
general properties of the linear perturbations. % that will be relevant
...
disc.%  In our disc models with $c_s^2\propto R^{-q}$, we expect the
% growth rate of such an instability to scale with $q$. 

{\bf 
  It is simplest to demonstrate instability by determining  
  the sign of the angular momentum density 
  $\jlin$ and the background torque $T_\mathrm{BG}$ for perturbations
  in the local approximation, which we review next. 
  % depends on the
  % type of perturbation being considered. 
  We then show that low-frequency perturbations ($|\omega|\ll\Omega$)
  are unstable when there a negative radial 
  temperature gradient is imposed.  
}

\subsection{Angular momentum density of  non-axisymmetirc low-frequency modes}
{\bf We now specialise to low-frequency modes and first determine the sign of its
  angular momentum density.} From Eq. \ref{lin_ang_mom_cons} and assuming a time-dependence of the
...
Thus, low-frequency non-axisymmetric modes ($m\geq1$)



Thus, low-frequency $m=1$ modes generally have negative
angular momentum. These are the modes observed in our simulations. 
If the mode loses (positive) angular momentum 
...
Note that in the local approximation, only axisymmetric perturbations
...
%($\Omega_p < \Omega$), grows slowly ($\gamma\ll\Omega_p$) and its
%spatial form is consistent local theory (\S\ref{fargo_m1}). 
%{\bf
%  We first regard a low-frequency, $m=1$ disturbance 
 % as a mode within the framework of local theory, satisfying the
 % % temperature gradient ignored in local theory
 % dispersion relation Eq. \ref{dispersion}. }
%To demonstrate possible
%instability, 
{\bf 
Here we show that the torque density acting on a low-frequency, $m=1$
...
momentum. {\bf Then} if it is a trailing spiral and $dc_s^2/dR<0$, as is typical
...
\begin{align}
  2\gamma \sim q\frac{c_s^2}{R}\frac{k}{\Omega} \sim q h
  \left(kH\right)\Omega.  
\end{align}
...
{\bf It is important to note that this growth is
due to the temperature gradient and does not depend explicitly on
...
\end{align}
where we used $c_s/R\sim h\Omega$. 
...
%For our fiducial disc models with
...
%above discussion is generic for low-frequency non-axisymmetric modes,
%but illustrate with a concrete example
We adopt computational units such that
