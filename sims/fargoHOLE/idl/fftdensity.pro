pro fftdensity,  r0=r0, loc=loc, start=start, finish=finish, xrange=xrange, yrange=yrange, rrange=rrange, azimode=azimode, smallh=smallh  
...
;;;;;;;;;;
  ...
  kappa1d_time = dblarr(finish-start+1,nrad)
  ...
  mode_ilr    = dblarr(nmodes,finish-start+1)
  mode_ilr_ex = dblarr(nmodes,finish-start+1)
...
;mode ilr
        test = m*omega1d - omega - kappa1d
        temp = min(abs(test),grid)
        ilr = radius(grid)
        mode_ilr(m-1, k-start) = ilr/r0
...
;average growth rate 
     ...
     print, 'time, amp, rc, growth ', time_norm(k-start), mode_corot(azimode-1, k-start), mode_growth(azimode-1, k-start);,  mode_ilr(azimode-1, k-start)
  endfor 
...
;explicit rate calculation for azimode  
  ...
  for i=0, finish-start do begin
    ...
    ;mode ilr
     test = azimode*omega1d_time(i,*) - omega - kappa1d_time(i,*)
     temp = min(abs(test),grid)
     ilr = radius(grid)/r0
     
     print, time_norm(i), rc/r0, growth_ex(i)/omega0, omega;, ilr, test(grid-1), test(grid+1)
  endfor
