...
pro vortgrad, loc=loc, start=start, finish=finish,plotrange0=plotrange0,ct=ct,out=out $
...
dvort_dr = dblarr(nsec,nrad-2)
d2vort_dr = dblarr(nsec,nrad-2)
d2vort_dphi = dblarr(nsec,nrad-2)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;laplacian on vorticity;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
for i=0, nsec-1 do begin
    dvort_dr(i,*) = deriv(rmed(0:nrad-3),vort(i,*))
    d2vort_dr(i,*) = deriv(rmed(0:nrad-3), rmed(0:nrad-3)*dvort_dr(i,*))/rmed(0:nrad-3)
endfor

for j=0, nrad-3 do begin
    d2vort_dphi(*,j) = deriv(azi,deriv(azi, vort(*,j)))/rmed(j)
endfor

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;NOW DO THE POLAR CONTOUR PLOTS;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
data = d2vort_dr + d2vort_dphi ;vort(0:nsec-1,0:nrad-3)/sigma(0:nsec-1,1:nrad-2)
...
levels=plotrange(0)+(plotrange(1)-plotrange(0))*(dindgen(96)/96.)
...
levels=plotrange(0)+(plotrange(1)-plotrange(0))*(dindgen(96)/95.)

device, filename=filepath(strcompress('vortgrad_'+ks+'.ps',/remove_all),root_dir='.',subdir=[location])$
,/color, bits_per_pixel=8,xsize=12, ysize=14
contour,dataplot,(rmed(1:nrad-2)-plrad)/rhill,azi/pi - 1.0 ,/fill,levels=levels,title=time+' orbits', $
xtitle='(r-r_p)/r_h', ytitle=textoidl('(\phi-\phi_p)/\pi'),charsize=1.5,xmargin=[7.0,6.0], ymargin=[3,2], $
