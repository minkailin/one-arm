...
/* void NonReflectingBoundary (Vrad, Rho, Energy) /\*ORIGINAL VERSION OF NRBC*\/ */
/*      PolarGrid *Vrad, *Rho, *Energy; */
/* { */
...
/*     dangle = (pow(Rinf[1],-1.5)-pow(RMAX,-1.5))/(.5*(cs0+cs1)); */
     ...
     PolarGrid *Vrad, *Vtheta, *Rho, *Energy;
{
  ...
  if (CPU_Rank == 0) { /*ANTARES NRBC...mimic what they did for outer boundary*/
    cs0 = 0.0;
    cs1 = 0.0;
    for (j=0; j<ns; j++) {
      cs0 += cs[j];
    ...
    /* /\*Ghost ring, assume keplerian and NO radial velo.*\/ */
/*     density0 = SigmaMed[0]; */
/*     urad0 = 0.0; */
/*     utheta0 = pow(Rmed[0],-0.5); */
     
#pragma omp parallel for private(l,jp,lp, dW, dU, dV, z1, z2, z3)
    for (j = 0; j < ns; j++) {
      l = j+i*ns; /*active*/
      lp= j; /*ghost*/
      
      rho[lp] = sqrt(density0*rho[l])*exp((urad0+vr[l])/2.0/csavg);
      vr[lp]  = 0.5*csavg*log(density0/rho[l]) + 0.5*(urad0 - vr[])  


      vt[lp]  = (density0*utheta0 + utheta0*z1 + utheta0*z3 + z2)/rho[lp];
...
#pragma omp parallel for private(l,jp,lp,vr_med, dW, dU, dV, z1, z2, z3)
    for (j = 0; j < ns; j++) {
      ...
      if((urad+csnrm2) < 0.0) z3 = 0.0;
      if(urad < 0.0) z2 = 0.0;
      if((urad-csnrm2) < 0.0) z1 =0.0;
