...
real FViscosity (rad)
     real rad;
{
  real viscosity, fr, fz1, fz2, drnu, drnu1;
  int i = 0;
...
*/
  ...
  //step function
//  viscosity *= Sigma(1.0)/Sigma(rad);
//  drnu = fabs(DNU*ASPECTRATIO*RZERO);
//  drnu1= fabs(DNU*ASPECTRATIO*RONE);
//  fr = 0.5*(1.0 + tanh((rad - RZERO)/drnu ) );
//  fz1 = fr + (1.0 - fr)*ANU;
//  fr = 0.5*(1.0 + tanh((rad - RONE)/drnu1 ) );
//  fz2 = fr*ANU + (1.0 - fr);
//  return viscosity*fz1*fz2;

//no inner torque
//  viscosity = (RMIN*RMIN*omega0(RMIN) - rad*rad*omega0(rad))*VISCOSITY*Sigma(RMAX)*pow(RMAX,3.0)*domega0(RMAX);
//  viscosity/= Sigma(rad)*pow(rad,3.0)*domega0(rad)*(RMIN*RMIN*omega0(RMIN) - RMAX*RMAX*omega0(RMAX));

//no vr 
   viscosity  = VISCOSITY*Sigma(RMAX)*pow(RMAX,3.0)*domega0(RMAX);
   viscosity /= Sigma(rad)*pow(rad, 3.0)*domega0(rad); 
...
#pragma omp for nowait
	...
	l = j+i*ns;
//        viscosity = FViscosity (Rmed[i]); 
//        viscosity *= pow(ANU, 1.0 - rho[l]/Sigma(Rmed[i]));
...
#pragma omp for
	...
	ljmim=ljm-ns;
//        viscosity = FViscosity (Rmed[i]);
//        viscosity *= pow(ANU, 1.0 - 0.25*(rho[l]+rho[lim]+rho[ljm]+rho[ljmim])/Sigma(Rmed[i]));
