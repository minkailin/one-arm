pro pdisk_domega, loc=loc, start=start, finish=finish, nopert=nopert, xrange=xrange, yrange=yrange, $
        hold=hold, ct=ct, zslice=zslice, plotrange=plotrange, r0=r0, aziline=aziline,phi0=phi0, $
...
for i=start, finish do begin
   ...
   for j=0, nx2-1 do begin
      for ii=0, nx1-1 do begin
         bigR = x1(ii)*sin(x2(j))
         vx3(ii,j,*)/=bigR ;this is Omega
      endfor
   endfor
   
   for k=0, nx3-1 do begin
      for j=0, nx2-1 do begin
         vx3(*,j,k) = deriv(alog(x1), alog(vx3(*,j,k)))
      endfor
   endfor
   data(*,*) = vx3(*,nzslice,*)
...
;   title  = title1+ textoidl(', (h.tan\theta)^{-1}=')+height_string
   ...
   set_plot,'ps'
   device, filename=strcompress('pdisk_domega_'+name2+'.ps',/remove_all) $
