...
static void TotalFlux (const State_1D *, double *, int, int, Grid *, const Data *);
...
void RightHandSide (const State_1D *state, Time_Step *Dts, 
              int beg, int end, double dt, Grid *grid, const Data *d)
/*! 
 *
...
/* -------------------------------------------------
   ...
   TotalFlux(state, gPhi, beg-1, end, grid, d);
  #endif
...
void TotalFlux (const State_1D *state, double *gPhi,
                int beg, int end, Grid *grid, const Data *d)
/*!
...
#endif
{
  ...
  double *x1p, *x2p, *x3p;
   ...
   wA = FARGO_GetVelocity();
  #endif
   double pot, pot1;//self-gravity. only for spherical
  ...
  if (g_dir == IDIR){ 
    ...
    /* -- gravitational potential -- */
       ...
       //add self-potential
       pot = d->Phi[*g_k][*g_j][i];
       pot1= d->Phi[*g_k][*g_j][i+1];
       gPhi[i]+= pot + (pot1 - pot)*(x1p[i] - x1[i])/(x1[i+1]-x1[i]);
    ...
    /* ----------------------------------------------------
       ...
       //add self-potential
       pot = d->Phi[*g_k][i][*g_i];
       pot1= d->Phi[*g_k][i+1][*g_i];
       gPhi[i]+= pot + (pot1 - pot)*(x2p[i] - x2[i])/(x2[i+1]-x2[i]);
    ...
    /* ----------------------------------------------------
       ...
       //add self-potential
       pot = d->Phi[i][*g_j][*g_i];
       pot1= d->Phi[i+1][*g_j][*g_i];
       gPhi[i]+= pot + (pot1 - pot)*(x3p[i] - x3[i])/(x3[i+1]-x3[i]);
