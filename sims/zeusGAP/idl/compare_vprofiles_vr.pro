...
pro compare_vprofiles_vr, cases=cases, start=start, finish=finish, rslice=rslice $
             , azislice=azislice $
...
dataplot = vr_1d*scale

if not keyword_set(ytitle) then ytitle = ''

zaxis = 1d0/(smallh*tan(theta))
set_plot, 'ps'
device, filename=strcompress('compare_vprofiles_vr'+string(start(0),format='(I03)')+'.ps',/remove_all) $ ;this is v_cylindrad
