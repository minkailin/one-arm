...
pro vort_xy, loc=loc, start=start, finish=finish, zslice=zslice $
             ,vertavg=vertavg, log=log, plotrange0=plotrange0, smooth=smooth $
             ,xtickinterval=xtickinterval, nopert=nopert, xrange=xrange,yrange=yrange $
             , basic=basic, mp=mp, name=name, vorten=vorten, linerange=linerange $
...
;        height = tan(!dpi/2d0 - theta(nzslice))/smallh
    ...
    if keyword_set(smooth) then begin
       array = where(dataplot le plotrange(0))
       dataplot(array) = plotrange(0)
    endif
    ...
    contour,dataplot,rplot,phi1,/fill,levels=levels,title=time+textoidl('P_0, ')+ $
    ...
    if keyword_set(linerange) then begin
       ...
       if (linerange(0) eq 0d0) then begin
          linerange(0) = min(data2d(r1:r2,*))
          linerange(1) = max(data2d(r1:r2,*))
       endif
       ...
       contour,dataplot(r1:r2,*), rplot(r1:r2) , phi1,levels=levels2,/overplot,c_color=255, thick=4
    endif


   if keyword_set(name) then begin
        xyouts, xrange(0), -0.95, textoidl(name),charsize=1.5, charthick=6, color=255, align=0.0
    endif 
   device,/close
